...
class Navigation {
    ...
    currentQuery() {
        if (this.options.useHashtagParser) {
            return (0, utilities_1.URLParamsObject)((location.hash || location.search || '').split('?')[1] || '');
        }
        else {
            return (0, utilities_1.URLParamsObject)(location.search);
        }
    }
    ...
    setOptions(options) {
        this.options = (0, utilities_1.UpdateObject)(this.options, options);
    ...
    parseElementCaptured(ev) {
        if (ev.target instanceof HTMLElement) {
            if (ev.target.hasAttribute('navigate:view') || ev.target.tagName == "A") {
                ...
                return (0, utilities_1.AscendingDOMPath)(ev.target, parent => parent.tagName == 'A' || parent.hasAttribute('navigate:view'));
            }
        }
        return undefined;
    }
    ...
    navigate(route, props, ev) {
        ...
        const query = hasProps ? `?${(0, utilities_1.ObjectURLParams)(props || {})}` : '';
